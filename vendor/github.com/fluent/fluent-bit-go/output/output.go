...
func FLBPluginSetContext(plugin unsafe.Pointer, ctx interface{}) {
	...
	// The C string is not managed by Go GC, so it will never be freed automatically.
	i := unsafe.Pointer(C.CString(""))
	// uintptr(i) produces the memory address of i. malloc() guarantees uniqueness of it.
	//
